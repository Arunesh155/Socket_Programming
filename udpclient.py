...
# Function to handle 2-player game using the server (UDP)
def send_choice_udp(choice):
    server_address = ('192.168.137.1', 12345)  # Replace with the actual server address and port
    ...
    try:
        ...
        messagebox.showinfo("Result", result.decode())
    finally:
        client_socket.close()
...
def main_menu():
    ...
    for member in member_names:
        tk.Label(window, text=f"{member}", font=("Arial", 10), fg="white", bg="#87CEEB").pack(side="bottom", pady=5, anchor="e")
